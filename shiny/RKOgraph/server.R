...
server <- function(input, output, session) {
  output$distPlot <- renderPlot({
    ...
    # -- For analysis on U/A/C/S
    if ( input$stress ) {df<-df[-c(2,3,6,7,11),]}
    # -- For analysus on U/A/C
    else {df <- df[-c(2,3,6,7,11,12,13,14),]}
    # -- For analysis on U/A/C and pooled_S
    #df2 <- as.data.frame(meta) %>% group_by(group) %>% summarize(dose=as.factor('200'), avg=mean(get(gene)),
    #                                                             sd=sd(get(gene)),
