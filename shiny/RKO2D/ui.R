...
ui <- fluidPage(
   ...
   # Application title
   titlePanel("RKO Dataset 2D Analysis"),
   ...
   sidebarLayout(
      sidebarPanel(
         ...
         fluidRow(column(checkboxInput("xlog1p", "log"),width=6),column(checkboxInput("xmagic", "imputed"),width=6)),
         selectInput("yaxis", "Choose Y axis: ",cols.continuous, selected=y.selected),
         textInput("ygene", "Type gene name for Y axis : ", ""),
         fluidRow(column(checkboxInput("ylog1p", "log"),width=6),column(checkboxInput("ymagic", "imputed"),width=6)),
         selectInput("coloraxis", "Choose Colors: ",cols.all, selected=color.selected),
         textInput("colorgene", "Type gene name for Color :", ""),
         fluidRow(column(checkboxInput("colorlog1p", "log"),width=6),column(checkboxInput("colormagic", "imputed"),width=6)),
